...
It uses the same four quantum states as BB84 but modifies the sifting procedure
to achieve resistance against photon number splitting (PNS) attacks.

Key difference from BB84: Instead of announcing bases, Alice announces pairs of
non-orthogonal states. Bob can only determine Alice's bit when he measured in
the correct basis AND can distinguish between the two announced states.
...
4. Sifting: Alice announces non-orthogonal state pairs; Bob keeps conclusive.
...
Sift rate: ~25% (vs BB84's ~50%), which is the tradeoff for PNS resistance.
"""
...
class SARG04Protocol(ProtocolPort):
    """SARG04 QKD protocol with step-by-step execution."""
    ...
    def run(self, num_qubits: int) -> ProtocolResult:
        """Execute the full SARG04 protocol."""
    ...
    def _step_preparation(self) -> StepResult:
        """Alice chooses random bits and bases, prepares qubits (same as BB84)."""
        self._alice_bits = [
        ...
        return self._make_step_result(
            ...
            "(rectilinear or diagonal), same as BB84.",
        )
    ...
    def _step_measurement(self) -> StepResult:
        """Bob measures each qubit in a randomly chosen basis (same as BB84)."""
        self._bob_bases = [
            self._randomness.random_basis() for _ in range(self._num_qubits)
        ]
        self._bob_results = []
        ...
        return self._make_step_result(
            ...
            "chosen basis, same as BB84.",
        )

    def _step_sifting(self) -> StepResult:
        """SARG04 sifting: Alice announces non-orthogonal state pairs.

        Instead of announcing bases (as in BB84), Alice announces a pair
        of non-orthogonal states {|psi>, |phi>} where |psi> is the state
        she actually sent and |phi> is a decoy from the other basis.

        Bob's measurement is conclusive when his basis matches Alice's AND
        his result allows him to distinguish between the announced states.
        Since Alice announces non-orthogonal pairs, Bob can only be
        conclusive ~50% of the time even when bases match.

        Overall sift rate: ~50% matching bases x ~50% conclusive = ~25%.

        In this simulation, we model the pair announcement by keeping
        matching-basis positions (like BB84) but applying an additional
        50% conclusive filter. This correctly models the ~25% sift rate
        and preserves eavesdropper detection through error rates.
        """
        self._matching_bases = []
        self._sifted_key_alice = []
        self._sifted_key_bob = []
        ...
        for i in range(self._num_qubits):
            alice_basis = self._alice_bases[i]
            alice_bit = self._alice_bits[i]
            bob_basis = self._bob_bases[i]
            bob_result = self._bob_results[i]

            if alice_basis != bob_basis:
                self._matching_bases.append(False)
                continue

            # Bases match. In SARG04, Alice announces a non-orthogonal
            # state pair. Bob can only conclusively determine Alice's bit
            # ~50% of the time, even with matching bases. We model this
            # with a random coin: conclusive when the coin gives ZERO.
            conclusive = self._randomness.random_bit() == BitValue.ZERO

            self._matching_bases.append(conclusive)
            if conclusive:
                self._sifted_key_alice.append(alice_bit)
                self._sifted_key_bob.append(bob_result)
        ...
        return self._make_step_result(
            f"Alice announces non-orthogonal state pairs instead "
            f"of bases. Bob identifies {n_sifted} of "
            f"{self._num_qubits} conclusive results "
            f"({rate:.0%} sift rate). Lower rate than BB84 "
            f"provides PNS attack resistance.",
        )
